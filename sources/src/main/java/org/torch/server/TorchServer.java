...
public final class TorchServer implements Runnable, org.torch.api.TorchReactor {
    ...
    public void convertMapIfNeeded(String world) {
        if (this.getAnvilFileConverter().isConvertable(world)) {
            ...
            this.getAnvilFileConverter().convert(world, new IProgressUpdate() {
                 ...
                 */
                @Override
                public void a(String message) {} // displaySavingString

                /**
                 * Updates the progress bar on the loading screen to the specified amount
                 */
                @Override
                public void a(int progress) { // setLoadingProgress
                 ...
                 */
                @Override
                public void c(String message) {} // displayLoadingString
            });
        }
    }
     ...
     */
    public List<String> tabCompleteCommand(ICommandListener sender, String message, @Nullable BlockPosition pos, boolean forceCommand) {
        return craftServer.tabComplete(sender, message, pos, forceCommand);
    }
    ...
    public boolean init() throws IOException {
        ...
        String startupCost = String.format("%.3fs", Double.valueOf(duration / 1.0E9D));

        logger.info("Ready for connections! ({})", startupCost);
