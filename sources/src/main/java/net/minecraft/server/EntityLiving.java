...
public abstract class EntityLiving extends Entity {
    ...
    public final Map<MobEffectList, MobEffect> effects = HashObjObjMaps.newMutableMap();
    ...
    public void U() {
        ...
        if (this.isAlive()) {
            if (this.inBlock()) {
                ...
                if (d0 < 0.0D) {
                    ...
                    if (d1 > 0.0D) {
                        this.damageEntity(DamageSource.STUCK, Math.max(1, MathHelper.floor(-d0 * d1)));
                    }
                }
            }
        }
        ...
        if (this.isAlive()) {
            if (this.a(Material.WATER)) {
                ...
                if (this.isPassenger() && this.bB() instanceof EntityLiving) {
                    this.stopRiding();
                }
            } else {
    ...
    public int getExpReward() {
        ...
        if ((this.lastDamageByPlayerTime > 0 || this.alwaysGivesExp()) && this.isDropExperience() && this.world.getGameRules().getBoolean("doMobLoot")) {
            return exp;
        } else {
            return 0;
        }
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (flag1) {
                    ...
                    if (entity1 != null) {
                        ...
                        this.aA = (float) (MathHelper.c(d1, d0) * 57.2957763671875D - this.yaw);
                        this.a(entity1, 0.4F, d0, d1);
                    } else {
                        this.aA = (int) (Math.random() * 2.0D) * 180;
                    }
                }
    ...
    protected float applyArmorModifier(DamageSource damagesource, float f) {
        if (!damagesource.ignoresArmor()) {
            ...
            f = CombatMath.a(f, this.getArmorStrength(), (float) this.getAttributeInstance(GenericAttributes.h).getValue());
        }
    ...
    public final int cc() {
        return this.datawatcher.get(EntityLiving.bq).intValue();
    }
    ...
    public void A(Entity entity) {
        ...
        if (!(entity instanceof EntityBoat) && !(entity instanceof EntityHorseAbstract)) {
            ...
            if (enumdirection != null) {
                ...
                AxisAlignedBB axisalignedbb = new AxisAlignedBB(d3 - d5 / 2.0D, this.getBoundingBox().b - entity.length, d4 - d6 / 2.0D, d3 + d5 / 2.0D, this.getBoundingBox().e - entity.length, d4 + d6 / 2.0D);
                int[][] aint1 = aint;
                int i = aint.length;
            ...
            if (entity instanceof EntityBoat) {
                ...
                f = 1.5707964F * (this.getMainHand() == EnumMainHand.RIGHT ? -1 : 1);
            }
            ...
            double d12 = this.locX + f1 * d0;
            double d13 = this.locZ + f2 * d0;

            this.setPosition(d12, entity.locY + entity.length + 0.001D, d13);
            if (this.world.a(this.getBoundingBox())) {
                this.setPosition(d12, entity.locY + entity.length + 1.001D, d13);
                if (this.world.a(this.getBoundingBox())) {
                    this.setPosition(entity.locX, entity.locY + this.length + 0.001D, entity.locZ);
                }
            }
        }
    }
        ...
        if (this.cu() || this.bA()) {
            ...
            if (this.isInWater() && (!(this instanceof EntityHuman) || !((EntityHuman) this).abilities.isFlying)) {
                ...
                if (this.hasEffect(MobEffects.LEVITATION)) {
                    this.motY += (0.05D * (this.getEffect(MobEffects.LEVITATION).getAmplifier() + 1) - this.motY) * 0.2D;
                } else {
    ...
    public void receive(Entity entity, int quantity) {
        if (!entity.dead) {
            ...
            if (entity instanceof EntityItem || entity instanceof EntityArrow || entity instanceof EntityExperienceOrb) {
                entitytracker.a(entity, (new PacketPlayOutCollect(entity.getId(), this.getId(), quantity)));
            }
        }
    }

    public boolean hasLineOfSight(Entity entity) {
        return this.world.rayTrace(new Vec3D(this.locX, this.locY + this.getHeadHeight(), this.locZ), new Vec3D(entity.locX, entity.locY + entity.getHeadHeight(), entity.locZ), false, true, false) == null;
    }
    ...
    public boolean cu() {
        return true;//!this.world.isClientSide;
    }
    ...
    protected void a(ItemStack itemstack, int i) {
        if (!itemstack.isEmpty() && this.isHandRaised()) {
            ...
            if (itemstack.n() == EnumAnimation.EAT) {
                ...
                this.a(SoundEffects.bO, 0.5F + 0.5F * this.random.nextInt(2), (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
            }
    ...
    public boolean j(double d0, double d1, double d2) {
        ...
        if (!flag) {
            ...
            for (int i = 0; i < 128; ++i) {
                ...
                double d7 = d3 + (this.locX - d3) * d6 + (random.nextDouble() - 0.5D) * this.width * 2.0D;
                double d8 = d4 + (this.locY - d4) * d6 + random.nextDouble() * this.length;
                double d9 = d5 + (this.locZ - d5) * d6 + (random.nextDouble() - 0.5D) * this.width * 2.0D;

                world.addParticle(EnumParticle.PORTAL, d7, d8, d9, f, f1, f2, new int[0]);
            }
